...
const KPICard: React.FC<KPICardProps> = ({ label, mainValue, subValue, tooltip, trend, trendColor = 'green', onClick }) => {
  ...
  return (
    <div 
      onClick={onClick}
      className={`bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col items-center justify-center text-center relative group ${onClick ? 'cursor-pointer hover:shadow-lg hover:border-blue-300' : 'hover:shadow-md'} transition-all`}
    >
      {tooltip && (
