...
import { Eye, Edit2, Trash2, Plus, Loader2, Search, Filter, Lock, Building2 } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (res.error) {
            ...
            setFormData({ name: '', username: '', email: '', password: '', phone: '', type: 'employee', status: 'active', companies: ['All'] });
            fetchUsers();
        }
      } catch (err) {
        console.error(err);
              ...
              {loading ? (
                ...
                currentUsers.map((user, idx) => (
                  <tr key={idx} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                       <div className="text-xs text-slate-600 font-bold">{Array.isArray(user.companies) ? user.companies.join(', ') : (user.companies || '-')}</div>
                    </td>
                    <td className="px-6 py-4">
