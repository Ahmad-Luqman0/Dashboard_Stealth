...
const UnregisteredSessionsPage: React.FC = () => {
 ...
 const [selectedSession, setSelectedSession] = useState<UnregisteredSession | null>(null);
  ...
  const handleRegisterUser = async (e: React.FormEvent) => {
    ...
    // Explicit Validation
    if (!registerForm.name.trim() || !registerForm.email.trim() || !registerForm.usertype_id) {
        setError('Please fill in all mandatory fields: Name, Email, and User Type.');
        return;
    }
    ...
    try {
      const result = await db.registerUserFromSession({
        ...
        shift_start: convertToUTC(registerForm.shift_start) || undefined,
        shift_end: convertToUTC(registerForm.shift_end) || undefined,
        breaktime_start: convertToUTC(registerForm.breaktime_start) || undefined,
        breaktime_end: convertToUTC(registerForm.breaktime_end) || undefined,
      ...
      if (result.error) {
        ...
        setRegisterForm({ 
            name: '', email: '', phone: '', usertype_id: '', company: '',
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 w-full max-w-md shadow-2xl border border-slate-100 dark:border-slate-800 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleRegisterUser} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Shift Start *</label>
                  <input
                    type="time"
                    step="1"
                    required
                    value={registerForm.shift_start}
                    onChange={(e) => setRegisterForm({ ...registerForm, shift_start: e.target.value })}
                    className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
                  />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Shift End *</label>
                  <input
                    type="time"
                    step="1"
                    required
                    value={registerForm.shift_end}
                    onChange={(e) => setRegisterForm({ ...registerForm, shift_end: e.target.value })}
                    className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Break Start</label>
                  <input
                    type="time"
                    step="1"
                    value={registerForm.breaktime_start}
                    onChange={(e) => setRegisterForm({ ...registerForm, breaktime_start: e.target.value })}
                    className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
                  />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Break End</label>
                  <input
                    type="time"
                    step="1"
                    value={registerForm.breaktime_end}
                    onChange={(e) => setRegisterForm({ ...registerForm, breaktime_end: e.target.value })}
                    className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
                  />
                </div>
              </div>
